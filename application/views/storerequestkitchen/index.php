<div class="container-fluid">
    ...
    <!-- Filter Tanggal -->
    <form method="GET" action="<?= base_url('storerequestkitchen/index') ?>" class="form-inline mb-3">
    ...
    <!-- Form Input -->
    <form id="form-add-request" method="POST" action="<?= base_url('storerequestkitchen/add') ?>">
        <div class="row">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" id="jenis_pengeluaran" name="jenis_pengeluaran" value="3">
                    <input type="text" class="form-control" value="KITCHEN" readonly>
                </div>
    ...
    <tbody>
        ...
        <?php foreach ($store_requests as $request): ?>
            <tr>
                ...
                <td>
                    <a href="<?= base_url('storerequestkitchen/edit/' . $request['id']) ?>" class="btn btn-warning btn-sm">Edit</a>
                    <a href="<?= base_url('storerequestkitchen/delete/' . $request['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</a>
                    <?php if ($request['status'] === 'pending'): ?>
                        <a href="<?= base_url('storerequestkitchen/verify/' . $request['id']) ?>" class="btn btn-success btn-sm">Verifikasi</a>
                        <a href="<?= base_url('storerequestkitchen/reject/' . $request['id']) ?>" class="btn btn-danger btn-sm">Tolak</a>
                    <?php endif; ?>
                </td>
            </tr>
        <?php endforeach; ?>
...
document.getElementById('cari-barang').addEventListener('input', function () {
    ...
    if (keyword.length > 0) {
        fetch('<?= base_url('StoreRequestKitchen/search_barang') ?>?keyword=' + encodeURIComponent(keyword))
...
document.getElementById('search-table').addEventListener('input', function () {
    ...
    if (query.length > 0) {
        fetch('<?= base_url('StoreRequestKitchen/filter_table') ?>?query=' + encodeURIComponent(query))
            .then(response => response.json())
            .then(data => {
                    ...
                    data.forEach(item => {
                        tableBody.innerHTML += `
                            <tr>
                                ...
                                <td>
                                    <a href="<?= base_url('storerequestkitchen/edit/') ?>${item.id}" class="btn btn-warning btn-sm">Edit</a>
                                    <a href="<?= base_url('storerequestkitchen/delete/') ?>${item.id}" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</a>
                                    ${item.status === 'pending' ? `
                                        <a href="<?= base_url('storerequestkitchen/verify/') ?>${item.id}" class="btn btn-success btn-sm">Verifikasi</a>
                                        <a href="<?= base_url('storerequestkitchen/reject/') ?>${item.id}" class="btn btn-danger btn-sm">Tolak</a>
                                    ` : ''}
                                </td>
                            </tr>
                        `;
