...
<body>
    ...
    $("#simpan-extra").click(function() {
        ...
        let list = selected.map(e =>
            `<li>➕ ${e.jumlah}x ${e.nama} <span class="float-right">Rp ${(e.harga * e.jumlah).toLocaleString('id-ID')}</span></li>`
        ).join("");
    ...
    $(document).on("click", ".pesanan-item", function() {
        ...
        $(this).addClass("selected");
        selectedOrderId = $(this).data("id");
        $("#ubah-pesanan").prop("disabled", false); // ⬅️ tombol aktif
    });

    ///// UBAH PESANAN
    $("#ubah-pesanan").click(function() {
        if (!selectedOrderId) {
            alert("Pilih pesanan yang ingin diubah.");
            return;
        }
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                $("#kode-voucher").val(res.kode_voucher || '');
                $("#nominal-diskon").text("Rp " + parseInt(res.diskon).toLocaleString("id-ID"));
                $("#total-harga").text("Rp " + parseInt(res.total_penjualan).toLocaleString(
                    "id-ID"));
                $("#total-bayar").text("Rp " + parseInt(res.total_pembayaran).toLocaleString(
                    "id-ID"));

                $("#transaksi-id").val(selectedOrderId); // simpan ID transaksi di hidden input
                $("#simpan-transaksi").hide(); // sembunyikan tombol buat baru
                $("#simpan-perubahan").show(); // tampilkan tombol simpan perubahan
                ...
                res.items.forEach(function(item) {
                    var row = `
                        <tr data-id="${item.pr_produk_id}" data-detail-id="${item.id}">
                            <td>${item.nama_produk}</td>
                            <td>Rp ${parseInt(item.harga).toLocaleString("id-ID")}</td>
                            <td><input type="number" class="form-control qty" value="${item.jumlah}" min="1" data-harga="${item.harga}"></td>
                            <td class="total">Rp ${parseInt(item.subtotal).toLocaleString("id-ID")}</td>
                            <td>
                                <button class="btn btn-sm btn-secondary btn-extra" data-id="${item.pr_produk_id}">Tambah Extra</button>
                                <!-- Extra list moved to its own row -->
                            </td>
                        ...
                        <tr class="extra-row" data-parent="${item.pr_produk_id}">
                            <td colspan="7">
                                <ul class="list-extra pl-4 mb-0 text-muted small" data-produk-id="${item.pr_produk_id}"></ul>
                            </td>
                        </tr>
                        `;
                    ...
                    if (item.extra && item.extra.length > 0) {
                        extraData[item.pr_produk_id] = item.extra;
                        let list = item.extra.map(e =>
                            `<li>${e.jumlah}x ${e.nama_extra}</li>`).join("");
                        $(`.list-extra[data-produk-id=${item.pr_produk_id}]`).html(list);
                    }
                });
    ...
    function getOrderData() {
        return {
            transaksi_id: $("#transaksi-id").val(), // ini kosong jika baru
            jenis_order_id: $("#jenis-order").val(),
            customer_type: $("#customer-type").val(),
            customer_id: $("#customer-id").val(),
            customer: $("#search-customer").val() || $("#walkin-customer-name").val(),
    ...
    $("#simpan-transaksi, #simpan-perubahan").on("click", function() {
        ...
        $("#order-list tr").each(function() {
            orderData.items.push({
                ...
                jumlah: $(this).find(".qty").val(),
                harga: $(this).find(".qty").data("harga"),
                subtotal: $(this).find(".total").text().replace("Rp ", "").replace(/\./g, ""),
                catatan: $(this).find(".catatan").val(),
                extra: extraData[$(this).data("id")] || []
            });
        });
