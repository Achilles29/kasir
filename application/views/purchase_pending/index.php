...
<div class="container-fluid">
    ...
    <div class="row">
    <!-- Form Input -->
    <form class="form-inline mb-3" method="post" action="<?= base_url('purchase_pending/add') ?>">
        <div class="form-group">
                ...
                <div class="form-group">
                    <label for="jenis_pengeluaran" class="mr-2">Jenis Pengeluaran:</label>
                ...
                <div class="form-group">
                    <label for="kuantitas" class="mr-2">Kuantitas:</label>
                ...
                <div class="form-group">
                        ...
                        <select id="metode_pembayaran" name="metode_pembayaran" class="form-control form-control-sm form-control-payment" required>
                            ...
                            <?php foreach ($metode_pembayaran as $metode): ?>
                                <option value="<?= $metode['id'] ?>"><?= $metode['nama_rekening'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
<div class="form-group">
...
<form method="GET" action="<?= base_url('purchase_pending/index') ?>" class="form-inline mb-3">
                ...
                <tbody>
                     <?php $no = 1; foreach ($purchases as $purchase): ?>
                        <tr>
                            ...
                            <td>
                                <a href="<?= base_url('purchase_pending/edit/' . $purchase['id']) ?>" class="btn btn-warning btn-sm">Edit</a>
                                <a href="<?= base_url('purchase_pending/delete/' . $purchase['id']) ?>" class="btn btn-danger btn-sm">Hapus</a>
                                <?php if ($purchase['status'] === 'pending'): ?>
                                    <a href="<?= base_url('purchase_pending/verify/' . $purchase['id']) ?>" class="btn btn-success btn-sm">Verifikasi</a>
                                    <a href="<?= base_url('purchase_pending/reject/' . $purchase['id']) ?>" class="btn btn-danger btn-sm">Tolak</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
document.getElementById('cari-nama-barang').addEventListener('input', function () {
    ...
    if (keyword.length > 0) {
        fetch(`<?= base_url('purchase_pending/search_barang') ?>?keyword=${encodeURIComponent(keyword)}`)
