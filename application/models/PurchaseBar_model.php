    ...
    class PurchaseBar_model extends CI_Model {
        ...
        ');
        $this->db->from('bl_purchase_bar');
        $this->db->join('bl_db_purchase', 'bl_purchase_bar.bl_db_purchase_id = bl_db_purchase.id', 'left');
        ...
        $this->db->join('bl_rekening', 'bl_purchase_bar.metode_pembayaran = bl_rekening.id', 'left');

            $this->db->where('bl_purchase_bar.tanggal >=', $start_date);
            $this->db->where('bl_purchase_bar.tanggal <=', $end_date);
        ...
        public function insert($data) {
            ...
            return $this->db->insert('bl_purchase_bar', $data);
        }
    ...
    public function get_all($start_date, $end_date, $limit, $offset) {
        ...
        ');
        $this->db->from('bl_purchase_bar');
        $this->db->join('bl_kategori', 'bl_purchase_bar.kategori_id = bl_kategori.id', 'left');
        $this->db->join('bl_tipe_produksi', 'bl_purchase_bar.tipe_produksi_id = bl_tipe_produksi.id', 'left');
        $this->db->join('bl_jenis_pengeluaran', 'bl_purchase_bar.jenis_pengeluaran = bl_jenis_pengeluaran.id', 'left');
        $this->db->join('bl_rekening', 'bl_purchase_bar.metode_pembayaran = bl_rekening.id', 'left');
        $this->db->where('bl_purchase_bar.tanggal >=', $start_date);
        $this->db->where('bl_purchase_bar.tanggal <=', $end_date);
        $this->db->limit($limit, $offset);
        $this->db->order_by('bl_purchase_bar.tanggal', 'DESC');
    ...
    {
        $this->db->where('id', $id);
        return $this->db->get('bl_purchase_bar')->row_array();
    }

    public function delete($id)
    {
        $this->db->where('id', $id);
        return $this->db->delete('bl_purchase_bar');
    }
        
public function update_status($id, $status) {
    $this->db->where('id', $id);
    $this->db->update('bl_purchase_bar', ['status' => $status]);
}
...
public function get_full_detail($id) {
    ...
    $this->db->join('bl_jenis_pengeluaran', 'bl_purchase_bar.jenis_pengeluaran = bl_jenis_pengeluaran.id', 'left');
    $this->db->join('bl_rekening', 'bl_purchase_bar.metode_pembayaran = bl_rekening.id', 'left');
    $this->db->where('bl_purchase_bar.id', $id);
...
public function update($id, $data) {
    $this->db->where('id', $id);
    return $this->db->update('bl_purchase_bar', $data);
}
